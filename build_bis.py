...
def get_items_from_eightyupgrades(url):
    ...
    slots = np.array(
        [int(re.findall("equippedSlotId&quot;:([0-9]*)", e)[0]) for e in elems]
    )
    ...
    def convert_tier_to_token(item_slot, item_name):
        assert (
            item_slot in [1, 3, 5, 10, 7]
            and "Conqueror" in item_name
            or "Triumph" in item_name
            or "Sanctified" in item_name
        )
        if (
            "Deathbringer" in item_name
            or "Aegis" in item_name
            ...
            "Worldbreaker" in item_name
            or "Siegebreaker" in item_name
            ...
            "Terrorblade" in item_name
            or "Darkruned" in item_name
            or "Kirin Tor" in item_name
            or "Nightsong" in item_name
        ):
            # Rogue / DK / Mage / Druid
            if item_slot == 1:  # Head
            ...
            "Kel'Thuzad" in item_name or "Turalyon" in item_name or "Velen" in item_name
        ):
            # Warlock / Paladin / Priest
            return 47557, "Regalia of the Grand Conqueror"

        elif (
            "Nobundo" in item_name or "Wrynn" in item_name or "Windrunner" in item_name
        ):
            # Shaman / Warrior / Hunter
            return 47558, "Regalia of the Grand Protector"

        elif (
            "VanCleef" in item_name
            or "Thassarian" in item_name
            or "Khadgar" in item_name
            or "Malfurion" in item_name
        ):
            # Rogue / DK / Mage / Druid
            return 47559, "Regalia of the Grand Vanquisher"

        elif (
            "Sanctified Dark Coven" in item_name
            or "Sanctified Lightsworn" in item_name
            ...
            "Sanctified Frost Witch" in item_name
            or "Sanctified Ymirjar Lord" in item_name
            ...
            "Sanctified Shadowblade" in item_name
            or "Sanctified Scourgelord" in item_name
            or "Sanctified Bloodmage" in item_name
            or "Sanctified Lasherweave" in item_name
        ):
            # Rogue / DK / Mage / Druid
...
def get_items_from_text(txt, ignore_len=False):
    ...
    names = np.array(
        [" ".join(line.split()[1:]) for line in txt.split("\n")], dtype="<U50"
    )
    if not ignore_len:
...
def build_bis(players):
    ...
    player_dfs = [
        pd.DataFrame({"player": p, "item_id": res[i][0], "item_name": res[i][1]})
