...
df_items["weight"] = (
    (1.0 + (df_items.slot == "Weapon 2H"))
    * (1.0 + 0.5 * (df_items.slot.isin(["Weapon 2H", "Weapon 1H", "Trinket"])))
    * (1.0 + (df_items.ilvl - 245) / (258 - 245))
...
def optimize_prios(
    df_source, resim=True, fixed_pre=None, fixed_post=None, epochs=80, target_temp=0.001
):
    ...
    def format_fixed_dict(fixed, df):
        ...
        for k in fixed:
            for i, p in enumerate(fixed[k]):
                fixed[k][i] = df.loc[
                    (df.item_id == k) & (df.player == p) & (~df.index.isin(stored))
                ].index[0]
    ...
    idxs_in_fixed = list(
        itertools.chain.from_iterable([fixed_pre[k] for k in fixed_pre])
    ...
    for step in range(epochs * total_links * resim):
        ...
        groups[item_id][first_id], groups[item_id][second_id] = (
            groups[item_id][second_id],
        ...
        if loss < min_loss:
            best_df = df.loc[
                list(itertools.chain.from_iterable(groups_with_fixed.values()))
            ]
            min_loss = loss
    ...
    non_lootable_sources = {
        ...
        "Emblems of Frost": [50454, 50456, 50458, 50356, 50461, 50459, 50457, 50455, 50355],
        "Craft": [45564, 45553, 45551, 45561, 45560, 47570, 47587, 49894, 49891],
        "P1": [
            40207,
            40321,
            40713,
        ...
        "PvP": [42853, 42608],
        "Reputation": [50400, 52572, 50402, 50398, 50404],
...
if __name__ == "__main__":
    df_priorities, temps, losses = optimize_prios(df_bis, fixed_pre={}, fixed_post={})
